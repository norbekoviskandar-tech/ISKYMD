...
export default function StemEditor({ editor }) {
  ...
  return (
    <section className="bg-[#141414] p-4 rounded-xl border border-[#2E2A1E] shadow-[0_15px_40px_rgba(0,0,94,0.04)] space-y-2">
      <div className="flex justify-between items-center ml-1">
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#D4AF37] font-mono block">Question Content</span>
      ...
      <textarea
        ...
        className={`bg-[#141414] text-[#F3EBD3] border border-[#2E2A1E] p-3 rounded-lg w-full resize-none min-h-[100px] text-sm font-medium focus:ring-8 focus:ring-[#D4AF37]/5 focus:border-[#D4AF37] outline-none transition-all ${errors.stem ? "border-red-500" : ""}`}
      />
      {stem.includes('|') && (
        <div className="p-3 bg-[#111111] rounded-lg border border-dashed border-[#3A3526]">
                ...
                <div className="flex bg-[#141414] dark:bg-zinc-800 rounded-lg p-1 border border-[#2E2A1E] dark:border-zinc-700 shadow-sm">
                  {['small', 'medium', 'large', 'default'].map(s => (
                    <button
                      ...
                      className={`px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-all ${stemImage.size === s ? 'bg-primary text-white shadow-sm' : 'text-zinc-400 hover:text-[#B8B09A] dark:hover:text-zinc-300'}`}
                    >
                ...
                <div className="flex bg-[#141414] dark:bg-zinc-800 rounded-lg p-1 border border-[#2E2A1E] dark:border-zinc-700 shadow-sm">
                  {[{ key: 'top', label: 'TOP' }, { key: 'bottom', label: 'BTM' }, { key: 'inline', label: 'INL' }].map(p => (
                    <button
                      ...
                      className={`px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-all ${stemImage.placement === p.key ? 'bg-[#D4AF37] text-black shadow-sm' : 'text-zinc-400 hover:text-[#B8B09A] dark:hover:text-zinc-300'}`}
                    >
                      {p.label}
                    </button>
                  ))}
